...
from src import schemas as venue_schema
...
def read_venue_by_id(venue_id: int, db: Session = Depends(database.get_db)):
...
def create_venue(venue: venue_schema.CreateVenue, db: Session = Depends(database.get_db)):
...
def update_venue(venue_id: int, venue: venue_schema.UpdateVenue, db: Session = Depends(database.get_db)):
...
def delete_venue_by_id(venue_id: int, db: Session = Depends(database.get_db)):
