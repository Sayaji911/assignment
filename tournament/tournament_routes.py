...
from src import schemas as tournament_schema
from src import database
router = APIRouter()

@router.get("/tournament/{tournament_id}", response_model=tournament_schema.BaseTournament, tags=['Tournament'])
def read_tournament_by_id(tournament_id: int, db: Session = Depends(database.get_db)):
...
def read_tournaments(skip: int = 0, limit: int = 100, db: Session = Depends(database.get_db)):
...
def create_tournament(tournament: tournament_schema.CreateTournament, db: Session = Depends(database.get_db)):
...
def update_tournament(tournament_id: int, tournament: tournament_schema.UpdateTournament, db: Session = Depends(database.get_db)):
...
def delete_tournament_by_id(tournament_id: int, db: Session = Depends(database.get_db)):
